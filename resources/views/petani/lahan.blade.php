        ...
        <div class="overflow-x-auto overflow-y-auto max-h-[calc(100vh-320px)] relative">
            <table class="w-full text-left border-collapse">
                <thead class="sticky top-0 z-10 bg-[#2D6A4F]">
                    ...
                    <th class="px-6 py-4 text-xs font-bold uppercase text-center">Produksi</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-green-50 transition">
                    ...
                    @php
                        $produksiSukses = $lahan->transaksi
                            ->whereIn('status_pembayaran', ['sukses', 'lunas'])
                            ->filter(function($t) use ($selectedTahun) {
                                return $t->created_at->year == $selectedTahun;
                            });
                        $produksiCount = $produksiSukses->count();
                        $produksiKg = $produksiSukses->sum('jumlah_beli');
                        $pengajuanAktif = $lahan->pengajuans->where('periode_id', $periodeAktif->id ?? 0)->last();
                        $periodeLabel = $pengajuanAktif && $pengajuanAktif->periode
                            ? ($pengajuanAktif->periode->nama_periode ?? ($pengajuanAktif->periode->tahun . ' - Musim ' . strtoupper($pengajuanAktif->periode->musim)))
                            : null;
                    @endphp
                    ...
                    <td class="px-6 py-4 text-center">
                        @if($produksiCount)
                            <div class="flex flex-col items-center gap-1">
                                <span class="font-black text-[#2D6A4F] text-sm">{{ $produksiCount }}x</span>
                                <span class="text-[10px] text-gray-500">{{ number_format($produksiKg, 1) }} Kg</span>
                            </div>
                        @else
                            <span class="text-[10px] text-gray-400 italic">Belum produksi</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-center">
                        @if($pengajuanAktif)
                    ...
                    @else
                        <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl">
                            <p class="text-[10px] text-amber-700 font-bold leading-tight text-center">
                                ...
                                Periode distribusi sedang tidak aktif. Penambahan lahan & pengajuan bibit dibuka saat musim tanam dimulai.
                            </p>
                        </div>
                    @endif
                </div>
